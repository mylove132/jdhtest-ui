 ...
 */
export default function (Api: Function, params = {}, options = {}) {
  const { pageNum = 1, pageSize = 10, watchFiled = true, needLoading = true, debounceTime = 600 } = options as any
  ...
  const tablePagination = reactive({
    ...
    size: 'small',
    showTotal: (total: number, range: any[]) => `共 ${total} 条记录，当前为第 ${range[0]}~${range[1]} 条记录`
  })
  /**
   *
   * @param {*} data 接口参数
  ...
  const initTableData = (data: any) => {
    ...
    Api()
      .then((res: { list: never[]; totalRecord: number }) => {
