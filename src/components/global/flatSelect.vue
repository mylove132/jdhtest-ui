<template>
  <div class="flatSelect">
    ...
    <div class="options">
      <slot>
        <span v-for="(option: any, index) in options" class="option" :class="isActive(option)" @click="selected(option)" :key="index">
          {{ option.label }}
        </span>
      </slot>
    </div>
...
const { multiple, value, theme } = toRefs(props) as any
