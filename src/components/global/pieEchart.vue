...
const initEchart = () => {
  ...
  myEchart = Echarts.init(main.value, undefined, { renderer: 'svg' })
  if (options.value) {
...
let selectedLengend: any = null // 当前可高亮数据（null 未为切换时候，代表所有）
let len = 0 // 数据长度
let currDataList: any[] = []
...
const initPieEvent = () => {
  ...
  myEchart.on('mouseover', (e: { name?: any; value?: any; dataIndex?: any }) => {
  ...
  myEchart.on('legendselectchanged', (e: { selected: any }) => {
    // 图表项，切换展示与否
    selectedLengend = e.selected
    // 重新开始轮播
    autoSwipe()
  })
  myEchart.on('click', (e: any) => {
    defineEmits(['clickItem', e])
  })
}
const mainAction = () => {
  const { series } = myEchart.getOption()
  currDataList = series[0].data.map((item: any, index: any) => ({
    ...item,
    dataIndex: index,
