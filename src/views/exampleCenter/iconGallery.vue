...
import iconJson from '@/iconList'
const loading = ref(false)
const iconList = reactive([...iconJson])
const seletedIconList = reactive<String[]>([])
...
const handelChange = (icon: { name: string; type: string }) => {
  let isCancel = false
...
const computedIconHtml = (item: { name: string; type: string }) => {
  const { type } = item
  return ` <icon-font type="${type}" />`
}
const delApi = (index: number, name: any) =>
  new Promise(resolve => {
    setTimeout(() => {
...
const handleDel = async (index: any, { name }: { name: string; type: string }) => {
  loading.value = true
